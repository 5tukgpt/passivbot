...
- **balance_override:** Hyperliquid's `marginSummary` only reports the perp sub-account balance. If funds sit in spot or vault, the bot under-sizes. `balance_override` in `config.live` hard-codes the effective balance so sizing uses the intended capital. Current value: `1146.0` (matches live HL spot USDC $1146.37 as of 2026-05-12).
